...
(async () => {
    ...
    try {
        ...
        const result = await page.evaluate(async () => {
            ...
            const originals = [...document.querySelectorAll('link:not([data-neo-stylesheet])')].map(el => el.media);
        ...
        assert.deepEqual(result, {staged:'rgb(255, 255, 255)',active:'rgb(32, 35, 34)',lazyMedia:'print',afterLazy:'rgb(32, 35, 34)',restored:'rgb(255, 255, 255)',restoredLazyMedia:null,originals:['screen','print',''],scriptCount:0,darkTarget:'dark',lightTarget:null,disabledTarget:null});
        ...
        assert.deepEqual(failed,{rejected:true,unchanged:'rgb(255, 255, 255)',leaked:0,retry:'rgb(32, 35, 34)'});
        ...
        assert.deepEqual(nativeDark, {light:'rgb(255, 255, 255)',restored:'rgb(32, 35, 34)',media:'screen'});
