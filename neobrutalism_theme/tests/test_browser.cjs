...
// Writes test settings and users. Run only against a disposable Odoo 17 database.
...
(async()=>{
 const browser=await chromium.launch({...(process.env.NEO_CHROME_PATH ? {executablePath:process.env.NEO_CHROME_PATH} : {}),headless:true});
 const errors=[];
 async function login(name,password){
  ...
  await page.waitForSelector('.o_neo_mode_toggle button',{timeout:90000});return page;
 }
 ...
 for(const [preset,color] of Object.entries(presets)){
  ...
  await admin.evaluate(()=>{
   document.querySelector('#neo-qa')?.remove();
   const host=document.createElement('div');host.id='neo-qa';host.style.cssText='position:fixed;right:20px;bottom:20px;width:600px;z-index:1000;padding:16px;background:var(--neo-surface);color:var(--neo-ink)';
  ...
  for(const mode of ['light','dark']){
   ...
   assert.equal(tokens.bg,mode==='dark'?'#202322':'#f7f5ee');
  ...
  await admin.evaluate(()=>document.querySelector('#neo-qa').remove());
 }
 ...
 assert.equal(await employee.locator('body').getAttribute('data-neo-mode'),'light');
