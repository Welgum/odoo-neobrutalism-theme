...
const root = {
    dataset: { unrelated: "keep", neoPalette: "pink" },
...
assert.equal(root.dataset.neoPalette, undefined, "legacy palette attributes are removed");
...
assert.deepEqual(normalizeThemeSettings(null), { accent_preset: "yellow" });
for (const invalid of ["#1122aa", "url(evil)", "constructor", "__proto__", ["blue"], null]) {
    assert.deepEqual(normalizeThemeSettings({ accent_preset: invalid }), { accent_preset: "yellow" });
}
...
for (const [preset, accent] of Object.entries(ACCENT_PRESETS)) {
    for (const mode of ["light", "dark"]) {
        styles.set("--neo-surface", "#ff0000");
        applyPreferences(root, { ...DEFAULTS, mode, palette: "pink" }, "light", { accent_preset: preset });
        assert.equal(root.dataset.neoMode, mode);
        assert.equal(styles.get("--neo-accent"), accent);
        assert.ok(contrast(styles.get("--neo-accent-ink"), accent) >= 4.5, `${preset} button/nav text contrast`);
        const ink = mode === "dark" ? "#f5f5ed" : "#161616";
        assert.ok(contrast(ink, styles.get("--neo-accent-soft")) >= 4.5, `${preset}/${mode} highlighted text contrast`);
        assert.deepEqual([...styles.keys()].sort(), ["--neo-accent", "--neo-accent-ink", "--neo-accent-soft", "--unrelated"],
