...
// Writes local fixtures. Use only a disposable Odoo 17 database with Contacts,
// Discuss, CRM, Calendar and Project installed; never a production database.
const assert = require('node:assert/strict');
const { chromium } = require('playwright');
const origin = process.env.NEO_TEST_URL || 'http://127.0.0.1:18067';
...
(async () => {
 const browser = await chromium.launch({headless:true,...(process.env.NEO_CHROME_PATH ? {executablePath:process.env.NEO_CHROME_PATH} : {})});
 try {
  ...
  const setMode = async mode => {
   if(await page.locator('body').getAttribute('data-neo-mode')!==mode) await page.locator('.o_neo_mode_toggle button').click();
  ...
  await page.route('**/neobrutalism_theme.assets*.css',route=>route.abort());
  ...
  for(const selector of ['.o-mail-DiscussSidebarChannel > span.text-truncate','.o-mail-Message-author','.o-mail-Message-body','.o-mail-Message-date','.o-mail-DateSection span']) await readable(selector,{surface:true});
  ...
  await readable('#neo-alert-qa h4, #neo-alert-qa p, #neo-alert-qa small');
  await page.evaluate(()=>document.querySelector('#neo-alert-qa').remove());
  ...
  await page.getByLabel('Use Neobrutalism theme').uncheck();await page.waitForSelector('body:not(.o_neo_theme)');
  assert.equal(await page.locator('link[data-neo-stylesheet][media=screen]').count(),0);
  await page.getByLabel('Use Neobrutalism theme').check();await page.waitForSelector('body[data-neo-mode=dark]');
  await readable('.o_neo_appearance',{surface:true});await page.getByRole('button',{name:'Done',exact:true}).click();
