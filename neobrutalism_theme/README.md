...
**Version:** `17.0.1.0.2` · **Odoo module:** `neobrutalism_theme` · **Dependencies:** `web`, `base_setup`.
...
![Odoo 17 Community Contacts list with the Neo Brutal backend theme, yellow accent, outlined buttons and light mode](static/description/backend-list.png)
...
1. Copy this module directory (or extract its distribution ZIP) into the **host add-ons directory already mounted into Odoo**. The resulting path must be:
   ...
   cd /path/to/your/compose-project
   mkdir -p addons
   unzip /path/to/neobrutalism_theme-17.0.1.0.2.zip -d addons
   ```
