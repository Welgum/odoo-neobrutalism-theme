# Odoo 17 Neobrutalism Backend Theme

**Neo Brutal** is an open-source **Odoo 17 Community backend theme** by [RivetFox](https://rivetfox.pro). It brings neobrutalism design to the Odoo ERP interface with bold outlines, solid shadows, seven accent color presets and personal **dark mode**.
...
**Version:** `17.0.1.0.0` · **Odoo module:** `neobrutalism_theme` · **Dependencies:** `web`, `base_setup`.
...
![Odoo 17 Community Contacts list with the Neo Brutal backend theme, yellow accent, outlined buttons and light mode](static/description/backend_screenshot.png)

### Dark mode: Odoo Discuss

![Odoo 17 Discuss in Neo Brutal dark mode with readable channel names, messages and a dark message composer](static/description/discuss-night.png)

These screenshots show actual Odoo 17 Community screens with demonstration data. More examples are included in the [theme screenshot gallery](static/description/).
...
1. Copy this module directory (or extract its distribution ZIP) into the **host add-ons directory already mounted into Odoo**. The resulting path must be:
   ...
   cd /path/to/your/compose-project
   mkdir -p addons
   unzip /path/to/neobrutalism_theme-17.0.1.0.0.zip -d addons
   ```
...
- [OCA web_dark_mode](https://github.com/OCA/web/tree/17.0/web_dark_mode): reviewed its asset-level approach to complete dark palettes. This implementation uses original Neo palette/code; no AGPL module code is included.
...
- [Odoo 17 web asset manifest](https://github.com/odoo/odoo/blob/17.0/addons/web/__manifest__.py)
- [Odoo 17 user menu registry](https://github.com/odoo/odoo/blob/17.0/addons/web/static/src/webclient/user_menu/user_menu_items.js)
- [Odoo 17 Dialog](https://github.com/odoo/odoo/blob/17.0/addons/web/static/src/core/dialog/dialog.js)
