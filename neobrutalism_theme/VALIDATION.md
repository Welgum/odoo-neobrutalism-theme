...
- Playwright checks cover every scene, the composition at loop reset (allowing only tiny antialiased-edge rounding differences), correct Odoo 16 labels, and locally decoded RivetFox artwork and fonts. Renderer pages make no external requests and report no JavaScript errors.
...
Visual review found that the old light Contacts image showed an unfiltered list with duplicate capture fixtures. It is now a byte-for-byte copy of this Odoo 16 branch’s existing complete twelve-record `tools/marketplace/screens/preset-yellow.png`. Cover, portrait and day/night assets were regenerated from that same-version capture.
...
- Captured fresh, actual Odoo 16 listing screenshots with fictional contacts and reviewed the day/night views, settings, forms, kanban, Appearance and versioned cover.
...
With the add-on on the Odoo 16 add-ons path and a disposable database:
...
NEO_TEST_URL=http://127.0.0.1:18066 NEO_TEST_DB=DISPOSABLE_DATABASE NEO_ALLOW_TEST_WRITES=1 node neobrutalism_theme/tests/test_browser.cjs
NEO_TEST_URL=http://127.0.0.1:18066 NEO_TEST_DB=DISPOSABLE_DATABASE NEO_ALLOW_TEST_WRITES=1 node neobrutalism_theme/tests/test_night_mode.cjs
```
...
Source references: [Odoo 16 web manifest](https://github.com/odoo/odoo/blob/16.0/addons/web/__manifest__.py), [settings view](https://github.com/odoo/odoo/blob/16.0/addons/base_setup/views/res_config_settings_views.xml), and [web client templates](https://github.com/odoo/odoo/blob/16.0/addons/web/views/webclient_templates.xml).
