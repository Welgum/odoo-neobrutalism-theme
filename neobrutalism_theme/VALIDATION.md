...
- Playwright checks cover every scene, the composition at loop reset (allowing only tiny antialiased-edge rounding differences), correct Odoo 17 labels, and locally decoded RivetFox artwork and fonts. Renderer pages make no external requests and report no JavaScript errors.
...
Visual review found that the old `backend-list.png` was an incomplete capture. It is now a byte-for-byte copy of this Odoo 17 branch’s existing, complete `tools/marketplace/screens/preset-yellow.png`; no screenshot from another Odoo version is substituted. Cover, portrait and day/night assets were regenerated. The capture helper now waits for list rows again after mode changes.
...
Validated on **2026-09-21** with official Odoo **17.0-20260908 Community** runtime, PostgreSQL 17, and headless Google Chrome. Tests used disposable databases only. Docker image storage was full, so the official ARM64 image layers were verified against their SHA-256 digests and extracted into temporary host storage, then run in isolated containers using that image’s own Python runtime and dependencies.

## Server and packaging

- Odoo discovers the add-on directly inside the repository's `neobrutalism_theme/` directory. The technical module name stays the same across Odoo 16–19.
...
- Verified version-specific dark CSS compile/load with only those dependencies, night mode survives reload, and day mode restores the original styles without browser errors.
...
- Captured fresh, actual Odoo 17 listing screenshots with fictional contacts and reviewed the day/night views, settings, forms, kanban, Appearance and versioned cover.

## Version-specific changes

- Use the Odoo 17 user service and its complete web asset bundle.
- Adapt browser checks and screenshot capture to Odoo 17 routes and component names.
...
With the add-on on the Odoo 17 add-ons path and a disposable database:
...
NEO_TEST_URL=http://127.0.0.1:18067 NEO_TEST_DB=DISPOSABLE_DATABASE NEO_ALLOW_TEST_WRITES=1 node neobrutalism_theme/tests/test_browser.cjs
NEO_TEST_URL=http://127.0.0.1:18067 NEO_TEST_DB=DISPOSABLE_DATABASE NEO_ALLOW_TEST_WRITES=1 node neobrutalism_theme/tests/test_night_mode.cjs
```
...
Source references: [Odoo 17 web manifest](https://github.com/odoo/odoo/blob/17.0/addons/web/__manifest__.py), [settings view](https://github.com/odoo/odoo/blob/17.0/addons/base_setup/views/res_config_settings_views.xml), and [web client templates](https://github.com/odoo/odoo/blob/17.0/addons/web/views/webclient_templates.xml).
